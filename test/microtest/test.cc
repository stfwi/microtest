...
bool test_basic_macros()
{
  ...
  // Unconditional pass/fail/warn
  {
    ...
    ok(test::num_warnings() == 1 && test::num_fails() == 1 && test::num_passed() == 1);
  }
  ...
  // checks on pass/fail using `if(test_expect_cond( ... )) { ... }`
  {
    ...
    ok(
      test::num_warnings() == 1 && test::num_fails() == 2 && test::num_passed() == 1
      && test_expect_cond_0_eq_1_has_failed);

    const bool test_expect_cond_1_eq_1_has_passed = test_expect_cond(1 == 1);
    ok(
      test::num_warnings() == 1 && test::num_fails() == 2 && test::num_passed() == 2
  ...
  // Expectation of an exception / expectation that no exception is thrown.
  {
    ...
    const auto testfn = []() {
      int no_exception_thrown = 0;
      (void)no_exception_thrown;
    };

    test_expect_except(throw "exception for test_expect_except()");
    ok(test::num_warnings() == 1 && test::num_fails() == 2 && test::num_passed() == 3);
    test_expect_except(testfn());
    ok(test::num_warnings() == 1 && test::num_fails() == 3 && test::num_passed() == 3);

    test_expect_noexcept(testfn());
    ok(test::num_warnings() == 1 && test::num_fails() == 3 && test::num_passed() == 4);

    test_expect_noexcept(throw "exception for test_expect_noexcept()");
    ok(test::num_warnings() == 1 && test::num_fails() == 4 && test::num_passed() == 4);
  }
  ...
  // Reset test statistics
  {
    ...
    test_reset();
    ok(test::num_warnings() == 0 && test::num_fails() == 0 && test::num_passed() == 0);
  }
...
bool test_relational_checks()
{
  ...
  const double const_dbl_1 = 1.0;
  const double const_dbl_2 = 2.0;

  // EQ (==)
  {
    ok(!test_expect_eq(0, 1));
    ok(test_expect_eq(1, 1));
    ...
    ok(test::num_fails() == 5);
    ok(test::num_passed() == 6);
    test_reset();
  }

  // NE (!=)
  {
    ok(test_expect_ne(0, 1));
    ok(!test_expect_ne(1, 1));
  ...
  // GT (>)
  {
    ok(!test_expect_gt(0, 1));
    ok(!test_expect_gt(1, 1));
    ok(test_expect_gt(2, 1));
    ok(test_expect_gt(const_int_two, int_one));
    ok(test_expect_gt(const_int_two, const_long_one));
    ok(test_expect_gt(const_int_two, long_one));
    ok(test_expect_gt(const_int_two, 0.2));
    ok(!test_expect_gt(const_dbl_1, const_int_one));
    ok(!test_expect_gt(const_int_one, 1));
    ok(!test_expect_gt(1, const_long_one));
    ok(test::num_fails() == 5);
  ...
  // LT (<)
  {
    ok(!test_expect_lt(2, 1));
    ok(!test_expect_lt(1, 1));
    ok(test_expect_lt(0, 1));
    ok(test_expect_lt(int_one, const_int_two));
    ok(test_expect_lt(const_long_one, const_int_two));
    ok(test_expect_lt(long_one, const_int_two));
    ok(test_expect_lt(0.2, const_int_two));
    ok(!test_expect_lt(const_int_one, const_dbl_1));
    ok(!test_expect_lt(const_int_one, 1));
    ok(!test_expect_lt(const_dbl_2, const_long_one));
    ok(test::num_fails() == 5);
    ok(test::num_passed() == 5);
    test_reset();
  }
  ...
  // GE (>=)
  {
    ok(test_expect_ge(2, 1));
    ok(test_expect_ge(1, 1));
    ok(!test_expect_ge(0, 1));
    ok(test_expect_ge(const_int_two, int_one));
    ok(test_expect_ge(const_int_two, const_dbl_2));
    ok(test_expect_ge(const_dbl_1, const_int_one));
    ok(!test_expect_ge(const_dbl_1, const_int_two));
    ok(test::num_fails() == 2);
    ok(test::num_passed() == 5);
    test_reset();
  }
  ...
  // LE (<=)
  {
    ok(!test_expect_le(2, 1));
    ok(test_expect_le(1, 1));
    ok(test_expect_le(0, 1));
    ok(test_expect_le(int_one, const_int_two));
    ok(test_expect_le(const_int_two, const_dbl_2));
    ok(test_expect_le(const_dbl_1, const_int_one));
    ok(!test_expect_le(const_dbl_2, const_int_one));
    ok(test::num_fails() == 2);
    ok(test::num_passed() == 5);
    test_reset();
  }
